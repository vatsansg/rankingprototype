...
def test_youth_happy_path_end_to_end(conn):
    ...
    cur = conn.cursor()
    cur.execute("SELECT status FROM dbo.ranking_run WHERE ranking_run_id=?", sen_run)
    ...
    cur.execute("SELECT * FROM dbo.ranking_run WHERE ranking_run_id=?", you_run)
    run = cur.fetchone()
    assert run.status == "SUCCEEDED"

    cur.execute("SELECT status FROM dbo.ranking_run_step WHERE ranking_run_id=?", you_run)
    steps = cur.fetchall()
    assert len(steps) == 11
    ...
    cur.execute(
        "SELECT age_category_code, ranking_points FROM dbo.main_ranking "
        "WHERE ranking_run_id=? AND competitor_id=91013 AND ranking_category='MD'",
        you_run,
    )
    doubles_row = cur.fetchone()
    ...
    cur.execute(
        "SELECT ranking_pos_age_category FROM dbo.main_ranking WHERE ranking_run_id=? AND ranking_category='MS' "
        "ORDER BY ranking_pos_age_category",
        you_run,
    )
    age_pos = cur.fetchall()
