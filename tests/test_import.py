...
def test_zpp_rows_always_import_at_zero_points(conn):
    ...
    row = conn.execute(
        "SELECT ranking_points, zero_point_penalty FROM new_events_results "
...
def test_cross_award_mirrors_senior_result_to_youth_at_5x(conn):
    conn.execute("INSERT INTO events (event_id, event_name, event_type_general_code) VALUES (1, 'E', 'WCH')")
    conn.execute(
        "INSERT INTO competitors (competitor_id, player_name, age_category_code) VALUES (1, 'Young Senior', 'U17')"
    )
    conn.execute(
        "INSERT INTO new_events_results (event_id, competitor_id, sub_event_code, result_position, "
    ...
    row = conn.execute(
        "SELECT ranking_points, cross_awarded_from_event_id FROM new_events_results WHERE category_code='YOU'"
    ).fetchone()
