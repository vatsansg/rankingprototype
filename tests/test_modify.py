...
def test_update_result_position_recomputes_points_and_logs_change(conn):
    ...
    updated = conn.execute(
        "SELECT result_position, ranking_points FROM new_events_results WHERE new_event_result_id=?",
    ...
    log = conn.execute(
        "SELECT * FROM new_events_results_modification_log WHERE new_event_result_id=?",
        (row["new_event_result_id"],),
    ).fetchone()
...
def test_zpp_row_stays_zero_points_even_if_position_changed(conn):
    ...
    zpp_row = conn.execute(
        "SELECT new_event_result_id FROM new_events_results WHERE competitor_id=90001 AND event_id=80005"
    ).fetchone()
