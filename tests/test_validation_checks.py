...
def test_duplicate_result_detected(conn):
    ...
    conn.cursor().execute(fixture_setup_sql("validation_failure"))
    ...
    assert any(not f.passed for f in duplicate_findings)

    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*) FROM dbo.ranking_validation_result WHERE ranking_run_id=? AND passed=0", run_id
    )
